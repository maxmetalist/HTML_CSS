...
## Очередная домашка
    Я чет вот подумал и добавил всё-таки шаблон категории. Без него всё
    ну совсем уж не логично. Вроде как мутим выбор по категориям, а как
    их выбирать и вообще смотреть, шаблона то такого нет...
### Главное
    По домашке надо было организовать типа слой бизнес-логики. Не совсем врубился
    что конкретно надо выноситьв этот слой и главное надо ли удалять вынесенные контроллеры
    из view...
### Redis установлен
    Я целиком его установил на комп и прописал  в PATH. Кэширование заделал 
    для каждого отдельного продукта. Кэшируется так же шаблон product_detail.
### Сервисная функция
    Она лежит по месту назначения в модуле services приложения catalog.
    В ней можно выбирать товары по категориям. Самих категорий пока нет
    (они не опубликованы),
    поэтому доступна панель для админов, чтоб исправлять, добавлять, удалять.
